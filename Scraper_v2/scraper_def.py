    ...
    with open(file_path, 'a', newline='') as file:
        ...
        for list_url in listing_url: 
            ...
            if state=="Novostavba":
                ...
                thewriter.writerow(information) #vypisovanie udajov inzeratov s novostavbami
                new_count += 1
            else:
                other_state.append([list_url, city, street, price, rooms, sale_rent, state, usable_area, land_area, eurm2])
                other_count += 1
                
            if isinstance(usable_area, int) and (isinstance(eurm2, float) or (isinstance(eurm2, int))) and state=="Novostavba" : #Osetrenie, aby sa vypocet vazeneho priemeru robil len vtedy, ak su oba udaje pritomne (uzitkova plocha a EUR/m2)
                use_area_list.append(usable_area)
                eurm2_list.append(eurm2)
            elif (isinstance(usable_area, int) and (isinstance(eurm2, float) or (isinstance(eurm2, int))) and state!="Novostavba"):
                other_use_area_list.append(usable_area)
                other_eurm2_list.append(eurm2)
            
        weigh_avg = sum(x * y for x, y in zip(use_area_list, eurm2_list))/sum(use_area_list)
        info=['Vazeny priemer', weigh_avg, 'Pocet prezrenych inzeratov', new_count] 
        thewriter.writerow(info)
        thewriter.writerow([])
        
        for insertion in other_state: #vypisanie udajov inzeratov, ktore nie su novostavby
            thewriter.writerow(insertion)
            
        weigh_avg = sum(x * y for x, y in zip(other_use_area_list, other_eurm2_list))/sum(other_use_area_list)
        info=['Vazeny_ priemer', weigh_avg, 'Pocet_prezrenych inzeratov', other_count]
        thewriter.writerow(info)
        thewriter.writerow([])
